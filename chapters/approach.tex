...
$penalty'_{attr-picked}$ จะต้องมากกว่าค่า penalty ของ weight ที่ถูกเปรีบเทียบด้วยเรียกว่า
$penalty_{attr-compare}$ ซึ่งก็คือสมการ
\begin{equation}
  penalty_{attr-compare} > penalty'_{attr-picked}
\end{equation}
และ $weight'_{attr-picked}$ จะต้องทำให้ค่า $penalty'_{attr-picked}$ มีค่าไม่น้อยไปกว่า $penalty_{attr-picked}$
ดังสมการ
\begin{equation}
...
เพื่อให้ weights คำนวณได้ง่าย ซึ่งจะใช้ L2 นอมัลไลเซชันในการคำนวณซึ่งจะเป็นดังสมการ
\begin{equation}
  weights = [weight_{messiness}, weight_{loudness}, weight_{do-not-disturb}]
\end{equation}
\begin{equation}
  magnitude = \sqrt{weight_{messiness}^2 + weight_{loudness}^2 + weight_{do-not-disturb}^2}
\end{equation}
\begin{equation}
  weights_{norm} = weights / magnitude
\end{equation}
...
หลังจากได้ $weights_{norm}$ เสร็จแล้วจะสามารถทำการปรับน้ำหนักซ้ำๆ ได้จนกว่าเจ้าของโปรไฟล์จะพอใจ
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=2.5in]{photo/diagram/finetune_flow.png}
  \end{center}
